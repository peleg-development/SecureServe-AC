...
function Cache.UpdateAll()
    ...
    for k, v in pairs(Cache.Values) do
        if k == "permissions" and type(v) == "table" then
            local copy = {}
            for kk, vv in pairs(v) do copy[kk] = vv end
            Cache.LastValues[k] = copy
        else
...
function Cache.Get(key, subKey)
    ...
    if key == "hasPermission" and subKey then
        ...
        if Cache.bootstrapping then
            if Cache.Values.permissions and Cache.Values.permissions[subKey] ~= nil then
