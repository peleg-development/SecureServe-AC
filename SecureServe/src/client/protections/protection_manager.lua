...
function ProtectionManager.start_memory_manager()
    ProtectionManager._memory_running = false
    Citizen.Wait(0)
    ProtectionManager._memory_running = true
    ProtectionManager.last_gc_time = GetGameTimer()

    ProtectionManager.memory_thread = Citizen.CreateThread(function()
        while ProtectionManager._memory_running do
            Citizen.Wait(15000)
...
function ProtectionManager.initialize()
    ...
    local protection_modules = {
        ...
        "anti_spectate", "anti_teleport", "anti_weapon_damage_modifier",
        ...
        "anti_super_jump", "anti_no_ragdoll", "anti_infinite_stamina", "anti_esp", "anti_input_block",
    }

    local blacklist_modules = {
    ...
    for _, module_name in ipairs(blacklist_modules) do
        local ok, mod = pcall(require, "client/blacklists/" .. module_name)
        if ok and type(mod) == "table" and type(mod.initialize) == "function" then
            ProtectionManager.register_protection(module_name, mod.initialize)
        else
            logger.warn("Blacklist module " .. module_name .. " missing or invalid")
        end
    end
    ...
    logger.info(("Initialized %d/%d protection modules"):format(count, #protection_modules + #blacklist_modules))
end
...
function ProtectionManager.initialize_heartbeat()
    AddEventHandler('playerSpawned', function()
        ...
        TriggerServerEvent("SecureServe:Heartbeat:Spawned")
    ...
    ProtectionManager.heartbeat_thread = Citizen.CreateThread(function()
        ...
        while true do
            Citizen.Wait(2000)
            ProtectionManager._heartbeat_tick = GetGameTimer()
            TriggerServerEvent('mMkHcvct3uIg04STT16I:cbnF2cR9ZTt8NmNx2jQS',
                Utils.random_key(math.random(15, 35)))
        end
    end)
    ...
    Citizen.CreateThread(function()
        Citizen.Wait(10000)
        local first_check = ProtectionManager._heartbeat_tick or 0
        while true do
            Citizen.Wait(8000)
            local now = GetGameTimer()
            local last = ProtectionManager._heartbeat_tick or first_check
            if last > 0 and (now - last) > 12000 then
                TriggerServerEvent("SecureServe:Server:Methods:PunishPlayer", nil,
                    "Heartbeat thread frozen (canary)")
                Citizen.Wait(5000)
            end
        end
    end)
    ...
    Citizen.CreateThread(function()
        TriggerServerEvent('SecureServe:Heartbeat:Loaded')
    end)
end
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    ProtectionManager._memory_running = false
    ProtectionManager._heartbeat_running = false
    ProtectionManager._proofs_running = false

    if ProtectionManager.memory_thread    then pcall(TerminateThread, ProtectionManager.memory_thread) end
    if ProtectionManager.heartbeat_thread then pcall(TerminateThread, ProtectionManager.heartbeat_thread) end
    if ProtectionManager.proofs_thread    then pcall(TerminateThread, ProtectionManager.proofs_thread) end
