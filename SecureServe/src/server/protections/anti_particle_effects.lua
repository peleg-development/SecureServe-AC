...
function AntiParticleEffects.initialize()
    AddEventHandler("ptFxEvent", function(sender, data)
        ...
        if data and data.effectHash and config_manager.is_blacklisted_particle(data.effectHash) then
            ban_manager.ban_player(sender, "Blacklisted Particle", {
                admin     = "Anti-Cheat System",
                time      = 2147483647,
                detection = "Used blacklisted particle effect: " .. tostring(data.effectHash),
            })
            CancelEvent()
            return
        end
        ...
        local max_per_sec = tonumber(config_manager.get_max_particles_per_second()) or 25
        if AntiParticleEffects.particle_tracking[sender] > max_per_sec * 5 then
            ban_manager.ban_player(sender, "Particle Effect Spam", {
                admin     = "Anti-Cheat System",
                time      = 2147483647,
                detection = "Too many particle effects: " .. AntiParticleEffects.particle_tracking[sender],
            })
            CancelEvent()
            return
        end
