...
function AntiExplosions.initialize()
    ...
    AddEventHandler('explosionEvent', function(sender, ev)
        ...
        if ev.ownerNetId == 0 then
            -- Server/scripted explosions: cancel only if blacklisted.
            if config_manager.is_blacklisted_explosion(ev.explosionType) then
                CancelEvent()
            end
            return
        end
        ...
        local v = config_manager.get_blacklisted_explosion(ev.explosionType)
        ...
        if v.scale and ev.damageScale and tonumber(ev.damageScale) and ev.damageScale > tonumber(v.scale) then
            ban(("Used boosted explosion (scale %.2f). %s"):format(ev.damageScale, explosion_info))
            return
        end
