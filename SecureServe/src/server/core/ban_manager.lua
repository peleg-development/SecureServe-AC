...
local function normalize_license(lic)
    if type(lic) ~= "string" then return nil end
    return (lic:gsub("license2?:", ""))
end

local function rebuild_index()
    BanManager.bans_index            = {}
    BanManager.bans_license_index    = {}
    for _, ban in ipairs(BanManager.bans) do
        if ban.identifiers then
            for k, id_value in pairs(ban.identifiers) do
                if type(id_value) == "string" then
                    -- direct identifier (license, license2, steam, discord,
                    -- xbl, live, fivem, ip, endpoint, ...) all indexed.
                    BanManager.bans_index[id_value] = ban
                    if k == "license" or k == "license2" then
                        local norm = normalize_license(id_value)
                        if norm then BanManager.bans_license_index[norm] = ban end
                    end
                elseif type(id_value) == "table" and k == "tokens" then
                    for _, token in ipairs(id_value) do
                        if type(token) == "string" then
                            BanManager.bans_index[token] = ban
                        end
                    end
                end
            end
...
function BanManager.check_ban(identifiers)
    ...
    if identifiers.license then
        local norm = normalize_license(identifiers.license)
        if norm then
            local ban = BanManager.bans_license_index and BanManager.bans_license_index[norm]
    ...
    if identifiers.tokens then
        for _, t in ipairs(identifiers.tokens) do
            local ban = BanManager.bans_index[t]
            if ban and not (ban.expires and ban.expires > 0 and os.time() > ban.expires) then
                return true, ban
            end
        end
...
function BanManager.ban_player(player_id, reason, details)
    ...
    if details.time and tonumber(details.time) then
        ...
        if mins == -1 then
            ...
            logger.warn(("Kicking %s (id %s) reason: %s"):format(player_name, pid, reason))

            if DiscordLogger and type(DiscordLogger.log_kick) == "function" then
                local ok, err = pcall(DiscordLogger.log_kick, pid, reason, details.admin or "Anti-Cheat System", false)
                if not ok then logger.error("DiscordLogger.log_kick failed: " .. tostring(err)) end
            end
    ...
    Citizen.CreateThread(function()
        ...
        if GetPlayerName(pid) then
            ...
            DropPlayer(tostring(pid), "You have been banned.\nReason: " .. reason .. expire_text)
        end
    end)
