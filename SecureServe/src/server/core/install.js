...
/**
 * Detects whether an error is the FiveM restricted-filesystem permission error
 * (common on Pterodactyl / sandboxed hosts where Node fs access is locked down).
 * @param {Error} err
 * @return {boolean}
 */
function isFsPermissionError(err) {
    ...
    return m.includes('allow-fs-read')
        || m.includes('access to this api has been restricted')
        || m.includes('fs.read')
        || m.includes('no device found')
        || err.code === 'ERR_ACCESS_DENIED';
}

function requestServerRestart(reason) {
    try {
        Logger.critical(`SecureServe needs a server restart: ${reason}`);
        Logger.critical(`Please restart the server (txAdmin -> Stop & Start) to finish applying the new key.`);
    } catch (_) {}
    try {
        StopResource && StopResource('SecureServe');
    } catch (_) {}
    try {
        if (typeof StopResource !== 'function' && typeof globalThis.ExecuteCommand === 'function') {
            globalThis.ExecuteCommand('stop SecureServe');
        }
    } catch (_) {}
}
...
function findAllResources() {
    ...
    try {
        ...
        if (isFsPermissionError(error)) {
            Logger.warning("Filesystem access is restricted on this host (--allow-fs-read).");
            Logger.warning("Cross-resource encryption install will be SKIPPED. The anticheat itself still works.");
            return resourceDirs; // vacio: el llamador lo maneja con gracia
        }
        Logger.error(`findAllResources error: ${error.message}`);
        return resourceDirs;
    }
...
function scanDirectory(dirPath, resourceDirs, depth = 0) {
    ...
    try {
        ...
        if (isFsPermissionError(error)) {
            // Host con filesystem restringido. No spamear: el llamador ya avisa.
            return;
        }
 ...
 * @return {string} Updated manifest content
 */
function updateManifestContent(manifestContent) {
    ...
    if (manifestContent.includes('fx_version')) {
        ...
        if (candidatePositions.length > 0) {
            insertPosition = Math.min(...candidatePositions);
        }
    }
        ...
        if (fs.existsSync(keyPath)) {
            ...
            if (isDefaultKey) {
                ...
                if (verifyKey === newKey) {
                    ...
                    requestServerRestart('new SecureServe key generated');
                    return true;
                } else {
...
function findServerCfg(startDir, maxDepth, currentDepth = 0) {
    ...
    try {
        ...
        for (const item of items) {
            if (item.startsWith('.') || item === 'node_modules' || item === 'cache' || item === 'resources') continue;
...
function installSecureServe() {
    ...
    if (!resources || resources.length === 0) {
        Logger.warning(`No external resources found/accessible. Skipping cross-resource encryption install.`);
        Logger.warning(`This is normal on hosts with restricted filesystem access (e.g. Pterodactyl).`);
        Logger.info(`SecureServe core protections are unaffected and will run normally.`);
        try {
            ensureSecureServeStartsFirst();
        } catch (e) {
            if (!isFsPermissionError(e)) Logger.error(`ensureSecureServeStartsFirst: ${e.message}`);
        }
        return;
    }
        ...
        setTimeout(() => {
            ...
            try {
                ...
                if (isFsPermissionError(e)) {
                    Logger.warning(`Cross-resource install skipped: filesystem is restricted on this host.`);
                    Logger.warning(`The SecureServe anticheat runs normally. To enable cross-resource`);
                    Logger.warning(`encryption, ask your host to allow --allow-fs-read for the FiveM process.`);
                } else {
