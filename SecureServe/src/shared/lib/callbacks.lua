...
if IS_SERVER then
    ...
    function Callbacks.trigger_client_callback(args)
        ...
        if tonumber(args.source) >= 0 then
            ...
            local cleaned = false
            local eventData
            eventData = RegisterNetEvent(('smp__callback_retval:%s:%s:%s'):format(args.source, args.eventName, ticket), function(packed)
                if eventCallback and prom.state == PENDING then eventCallback(table_unpack(msgpack_unpack(packed))) end
                if prom.state == PENDING then prom:resolve(table_unpack(msgpack_unpack(packed))) end
                if not cleaned and eventData then
                    cleaned = true
                    RemoveEventHandler(eventData)
                end
            end)
            ...
            local timeout_ms = (tonumber(args.timeout) or 30) * 1000
            ...
            SetTimeout(timeout_ms, function()
                ...
                if prom.state == PENDING or prom.state == REJECTED or prom.state == REJECTING then
                    if timedout then pcall(timedout, prom.state) end
            ...
            if not eventCallback then
                local ok, result = pcall(Citizen.Await, prom)
                if not cleaned and eventData then
                    cleaned = true
                    RemoveEventHandler(eventData)
                end
