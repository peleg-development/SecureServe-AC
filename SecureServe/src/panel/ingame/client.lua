...
RegisterNUICallback('SecureServe:Panel:Unban', function(data, cb)
    ...
    TriggerServerEvent('SecureServe:Panel:Unban', banId)
    cb('ok')
end)
...
RegisterNUICallback('spawnVehicle', function(data, cb)
    ...
    if SecureServe and SecureServe.Protection and SecureServe.Protection.BlacklistedVehicles then
        for _, v in pairs(SecureServe.Protection.BlacklistedVehicles) do
            if v.name and GetHashKey(v.name) == vehicleHash then
                ac_notify('Vehicle blacklisted: ' .. vehicleName)
                cb('ok')
                return
            end
        end
...
RegisterNUICallback('spawnObject', function(data, cb)
    ...
    if SecureServe and SecureServe.Protection and SecureServe.Protection.BlacklistedObjects then
        for _, v in pairs(SecureServe.Protection.BlacklistedObjects) do
            if v.name and GetHashKey(v.name) == objectHash then
                ac_notify('Object blacklisted: ' .. objectName)
                cb('ok')
                return
            end
        end
...
RegisterNUICallback('changePed', function(data, cb)
    ...
    if SecureServe and SecureServe.Protection and SecureServe.Protection.BlacklistedPeds then
        for _, v in pairs(SecureServe.Protection.BlacklistedPeds) do
            if v.name and GetHashKey(v.name) == pedHash then
                ac_notify('Ped blacklisted: ' .. pedModel)
                cb('ok')
                return
            end
        end
...
RegisterNetEvent('SecureServe:Panel:ReceivePlayers', function(playerList, requestId)
    SendNUIMessage({
        action = 'players',
        players = playerList
    })
...
RegisterNUICallback('SecureServe:Panel:RequestPlayers', function(data, cb)
    local reqId = math.random(100000, 999999)
    local p = promise.new()
    pendingPlayersPromise = { id = reqId, resolve = function(result) p:resolve(result) end }
    TriggerServerEvent('SecureServe:Panel:RequestPlayers', reqId)
...
RegisterNUICallback('SecureServe:Panel:Kick', function(data, cb)
    local playerId = data.playerId
    TriggerServerEvent('SecureServe:Panel:Kick', playerId)
    cb({ success = true })
end)

RegisterNUICallback('SecureServe:Panel:Ban', function(data, cb)
    local playerId = data.playerId
    TriggerServerEvent('SecureServe:Panel:Ban', playerId)
    cb({ success = true })
end)

RegisterNUICallback('spectatePlayer', function(data, cb)
    local playerId = data.playerId
    -- Pedimos al servidor que resuelva las coords del objetivo (puede estar
    -- lejos). El servidor responde con SecureServe:Panel:DoSpectate.
    TriggerServerEvent('SecureServe:Panel:RequestSpectate', playerId)
    cb({ success = true })
end)

-- Responde al servidor con nuestras coords actuales (el objetivo es quien
-- ejecuta esto). Usado por el flujo de spectate.
...
-- El admin recibe las coords del objetivo: se teletransporta cerca, espera a
-- que el ped del objetivo aparezca en su scope, y entra en modo espectador.
RegisterNetEvent('SecureServe:Panel:DoSpectate', function(targetServerId, coords)
    if not coords then
        ac_notify("Could not locate that player.")
        return
    end
    ...
    -- Subir un poco para no quedar dentro de geometria.
    SetEntityCoords(myPed, coords.x + 0.0, coords.y + 0.0, coords.z + 50.0, false, false, false, true)
    SetEntityVisible(myPed, false, false)
    FreezeEntityPosition(myPed, true)

    ac_notify("Loading player... wait a moment.")

    Citizen.CreateThread(function()
        local targetPed = 0
        local tries = 0

        -- Esperar a que el ped del objetivo se cargue en nuestro scope.
        while tries < 50 do  -- ~5s max
            local plyr = GetPlayerFromServerId(targetServerId)
            if plyr ~= -1 then
                targetPed = GetPlayerPed(plyr)
        ...
        if not targetPed or targetPed == 0 or not DoesEntityExist(targetPed) then
            -- Restaurar al admin si no se pudo.
            ...
            ac_notify("Could not load that player. Try again.")
            return
        end
        ...
        ac_notify("Spectating player " .. tostring(targetServerId) .. ". Press BACKSPACE to exit.")
...
RegisterNetEvent('SecureServe:Panel:ReceivePlayers', function(playerList)
...
RegisterNUICallback('executeServerOption', function(data, cb)
    if data.action == 'restart' then
        TriggerServerEvent('SecureServe:Panel:RestartServer')
    end
