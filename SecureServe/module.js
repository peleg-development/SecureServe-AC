// im not sure i did correctlly so dont install this module just yet until im sure this is ok :)
...
globalThis.onNet = (event_name, handler) => {
    ...
    _onNet(enc_event_name, function() {
        const src = global.source;
        if (GetPlayerPing(src) > 0 && decrypt(enc_event_name) !== "add_to_trigger_list" && decrypt(enc_event_name) !== "check_trigger_list") {
...
globalThis.emitNet = (event_name, target, ...args) => {
    const enc_event_name = encryptDecrypt(event_name);
    return _emitNet(enc_event_name, target, ...args);
};
